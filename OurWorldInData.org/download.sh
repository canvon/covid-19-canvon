...
then
	...
	cp -vi "$TMPOUT_FILENAME" "$ARCHIVE_DIR/${DEST_BASENAME}-${TIMESTAMP}.${DEST_EXTENSION}" || die "Copying downloaded file \"$TMPOUT_FILENAME\" to archive directory \"$ARCHIVE_DIR\" failed."
